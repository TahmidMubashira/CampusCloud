...
function AdminSidebar({ active, admin }: { active: string; admin: any }) {
  return (
    <aside style={{
      ...
      <div style={{ margin: "0 12px", borderTop: "1px solid #b8cdd9", paddingTop: "16px", display: "flex", alignItems: "center", gap: "10px" }}>
        <div style={{ width: "36px", height: "36px", borderRadius: "50%", background: "#6a8fa8", display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontWeight: 700, fontSize: "0.85rem" }}>
          {admin?.name ? admin.name.charAt(0).toUpperCase() : 'A'}
        </div>
        <div>
          <div style={{ fontWeight: 600, fontSize: "0.8rem", color: "#1a3a50" }}>{admin?.name || 'Admin User'}</div>
          <div style={{ fontSize: "0.68rem", color: "#6a8fa8" }}>{admin?.email || 'admin@uni.edu'}</div>
        </div>
      </div>
    </aside>
  );
...
function StatCard({ label, value, sub, icon }: { label: string; value: string | number; sub: string; icon: string }) {
  return (
    <div style={{ background: "#fff", borderRadius: "10px", padding: "18px 20px", flex: 1, border: "1px solid #dce8f0", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
      <div>
    ...
    try {
      ...
      const response = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${apiKey}`,
        {
          ...
          body: JSON.stringify({
            contents: [
              { role: 'user', parts: [{ text: 'You are CampusCloud AI Assistant for university platform administrators. Help with resource quality evaluation, content moderation decisions, engagement analytics, identifying high-quality academic resources, and platform management best practices. Be professional, concise, and data-driven.' }] },
              { role: 'model', parts: [{ text: 'Understood! I am CampusCloud Admin AI Assistant, ready to help with platform moderation and analytics. How can I assist you?' }] },
              ...chatHistory,
      ...
      if (data.error) { setMessages(prev => [...prev, { role: 'assistant', text: `❌ API Error: ${data.error.message}` }]); return; }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 140px)' }}>
      ...
      <div style={{ marginBottom: '16px', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: 'linear-gradient(135deg, #2e7da8, #1a3a50)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.2rem', flexShrink: 0 }}>🤖</div>
          <div>
            <h1 style={{ fontWeight: 700, fontSize: '1.3rem', color: '#1a3a50', margin: 0 }}>AI Admin Assistant</h1>
            <p style={{ color: '#7a9db5', fontSize: '0.75rem', margin: 0 }}>Powered by Google Gemini • Platform moderation & analytics support</p>
          </div>
        </div>
      </div>
      <div style={{ flex: 1, background: '#fff', borderRadius: '14px', border: '1px solid #dce8f0', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          {messages.map((msg, i) => (
            <div key={i} style={{ display: 'flex', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
              {msg.role === 'assistant' && (
                <div style={{ width: '30px', height: '30px', borderRadius: '50%', background: 'linear-gradient(135deg, #2e7da8, #1a3a50)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.85rem', flexShrink: 0, marginRight: '8px', marginTop: '4px' }}>🤖</div>
              )}
              <div style={{ maxWidth: '75%', padding: '10px 14px', borderRadius: msg.role === 'user' ? '18px 18px 4px 18px' : '18px 18px 18px 4px', background: msg.role === 'user' ? '#1a3a50' : '#f0f4f8', color: msg.role === 'user' ? '#fff' : '#1a3a50', fontSize: '0.85rem', lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>
                {msg.text}
              </div>
            </div>
          ))}
          {loading && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <div style={{ width: '30px', height: '30px', borderRadius: '50%', background: 'linear-gradient(135deg, #2e7da8, #1a3a50)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.85rem' }}>🤖</div>
              <div style={{ padding: '10px 14px', borderRadius: '18px 18px 18px 4px', background: '#f0f4f8', color: '#7a9db5', fontSize: '0.85rem' }}>✨ Thinking...</div>
            </div>
          )}
          <div ref={bottomRef} />
        </div>
        {messages.length === 1 && (
          <div style={{ padding: '0 16px 12px', display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
            {suggestedQuestions.map((q, i) => (
              <button key={i} className="admin-suggest-btn" onClick={() => setInput(q.replace(/^[^\s]+ /, ''))}
                style={{ padding: '5px 10px', borderRadius: '16px', border: '1px solid #dce8f0', background: '#f8fbfe', color: '#4a6a80', fontSize: '0.72rem', cursor: 'pointer', transition: 'background 0.15s' }}>
                {q}
              </button>
            ))}
          </div>
        ...
        <div style={{ padding: '12px 16px', borderTop: '1px solid #dce8f0', display: 'flex', gap: '10px', alignItems: 'flex-end' }}>
          <textarea className="admin-chat-input" value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKeyDown}
            placeholder="Ask about moderation, resource quality, analytics... (Enter to send)" rows={1}
            style={{ flex: 1, padding: '10px 14px', border: '1.5px solid #dce8f0', borderRadius: '10px', fontSize: '0.85rem', fontFamily: 'inherit', resize: 'none', lineHeight: 1.5, color: '#1a3a50', background: '#f8fbfe' }} />
          <button className="admin-send-btn" onClick={sendMessage} disabled={loading || !input.trim()}
            style={{ padding: '10px 18px', borderRadius: '10px', border: 'none', background: loading || !input.trim() ? '#b8cdd9' : '#2e7da8', color: '#fff', fontWeight: 700, fontSize: '0.85rem', cursor: loading || !input.trim() ? 'not-allowed' : 'pointer', transition: 'background 0.2s', flexShrink: 0 }}>
            {loading ? '⏳' : '➤'} Send
          </button>
        </div>
      </div>
...
export default function AdminPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [statsRes, pendingRes] = await Promise.all([
        fetch(`/api/admin/stats`, { headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' } }),
        fetch(`/api/admin/pending`, { headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' } }),
      ]);
  ...
  // ── handleReject now takes reason as a parameter — no window.prompt ────────
  const handleReject = async (id: number, title: string, reason: string) => {
    try {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#f0f4f8" }}>
      ...
      <main style={{ flex: 1, padding: "36px 40px", overflowY: "auto", position: "relative" }}>
        ...
        {toast && (
          <div style={{ position: "fixed", top: "24px", right: "24px", zIndex: 9999, background: "#1a3a50", color: "#fff", padding: "12px 20px", borderRadius: "8px", fontSize: "0.85rem", fontWeight: 500, boxShadow: "0 4px 16px rgba(0,0,0,0.15)" }}>
            {toast}
          </div>
        )}
        ...
        {currentView === 'ai' ? (
          ...
          <>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "28px" }}>
              ...
              <button onClick={handleLogout} style={{ background: "#fee2e2", border: "1.5px solid #fca5a5", color: "#dc2626", padding: "8px 20px", borderRadius: "8px", fontSize: "0.85rem", fontWeight: 600, cursor: "pointer" }}>
                Logout
              </button>
            </div>
            ...
            <div style={{ background: "#fff", borderRadius: "12px", padding: "24px", border: "1px solid #dce8f0" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
                <h2 style={{ fontWeight: 700, fontSize: "1rem", color: "#1a3a50", margin: 0 }}>Pending Resource Approvals</h2>
              ...
              {!loading && pending.map(item => (
                <div key={item.id} style={{ marginBottom: "10px" }}>
                  {/* Resource row */}
                  <div style={{ display: "flex", alignItems: "center", gap: "14px", padding: "14px 16px", background: "#f5f9fc", borderRadius: rejectingId === item.id ? "8px 8px 0 0" : "8px", border: "1px solid #e4eef5", borderBottom: rejectingId === item.id ? "none" : "1px solid #e4eef5", flexWrap: "wrap" }}>
                    ...
                    <button
                      ...
                      style={{ background: "#e6f5ee", color: "#2a8a5a", border: "1px solid #a8d9be", borderRadius: "6px", padding: "5px 14px", fontSize: "0.75rem", fontWeight: 600, cursor: "pointer", marginRight: "6px" }}
                      onMouseEnter={e => e.currentTarget.style.background = '#c6ebd8'}
                      onMouseLeave={e => e.currentTarget.style.background = '#e6f5ee'}>
                      ✓ Accept
                    </button>

                    {/* Reject button — toggles inline input */}
                    {rejectingId !== item.id && (
                      <button
                        onClick={() => { setRejectingId(item.id); setRejectReason(''); }}
                        style={{ background: "#fdf0ee", color: "#c0442a", border: "1px solid #f0b8ae", borderRadius: "6px", padding: "5px 14px", fontSize: "0.75rem", fontWeight: 600, cursor: "pointer" }}
                  ...
                  {/* Inline rejection reason input — appears when Reject is clicked */}
                  {rejectingId === item.id && (
                    <div style={{ padding: "12px 16px", background: "#fff9f9", border: "1px solid #fecaca", borderTop: "none", borderRadius: "0 0 8px 8px", display: "flex", gap: "8px", alignItems: "center", flexWrap: "wrap" }}>
                      <span style={{ fontSize: "0.78rem", color: "#dc2626", fontWeight: 600, flexShrink: 0 }}>💬 Reason:</span>
                      <input
                        autoFocus
                        ...
                        onKeyDown={e => {
                          if (e.key === 'Enter' && rejectReason.trim()) {
                            handleReject(item.id, item.title, rejectReason.trim());
                            setRejectingId(null);
                            setRejectReason('');
                          }
                          if (e.key === 'Escape') { setRejectingId(null); setRejectReason(''); }
                        }}
                        style={{ flex: 1, minWidth: "200px", padding: "6px 10px", borderRadius: "6px", border: "1px solid #fecaca", fontSize: "0.78rem", outline: "none" }}
                      />
                      <button
                        ...
                        style={{ padding: "6px 14px", borderRadius: "6px", border: "none", background: rejectReason.trim() ? "#dc2626" : "#fca5a5", color: "#fff", fontSize: "0.75rem", fontWeight: 600, cursor: rejectReason.trim() ? "pointer" : "not-allowed" }}>
                        Confirm
                      </button>
                      <button
                        onClick={() => { setRejectingId(null); setRejectReason(''); }}
                        style={{ padding: "6px 12px", borderRadius: "6px", border: "1px solid #dce8f0", background: "transparent", color: "#7a9db5", fontSize: "0.75rem", cursor: "pointer" }}>
                        Cancel
                      </button>
                    </div>
                  )}
